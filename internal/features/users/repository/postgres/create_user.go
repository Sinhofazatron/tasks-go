...
func (r *UsersRepository) CreateUser(ctx context.Context, user domain.User) (domain.User, error) {
	...
	err := row.Scan(&userModel.ID, &user.Version, &user.FullName, &user.PhoneNumber)
	if err != nil {
		return domain.User{}, fmt.Errorf("scan error: %w", err)
	}

	userDomain := domain.NewUser(userModel.ID, user.Version, user.FullName, user.PhoneNumber)
